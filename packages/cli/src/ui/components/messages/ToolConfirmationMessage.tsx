...
> = ({
  ...
  const { question, bodyContent, options, handlesOwnUI } = useMemo(() => {
    ...
    // Flag for confirmation types that render their own question/options UI
  ...
  return (
    ...
    >
      {/* Body Content (Diff Renderer, Command Info, or AskUserDialog) */}
      ...
      {/* Confirmation Question - skip for types that handle their own UI */}
      {!handlesOwnUI && (
      ...
      {/* Select Input for Options - skip for types that handle their own UI */}
      {!handlesOwnUI && (
        <Box flexShrink={0}>
          <RadioButtonSelect
            items={options}
